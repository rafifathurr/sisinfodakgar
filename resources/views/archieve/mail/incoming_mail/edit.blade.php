    ...
    <section class="px-4">
        <div class="col-md-12 mb-5">
            <form class="forms-sample" method="post"
                action="{{ route('archieve.mail.incoming-mail.update', ['id' => $incoming_mail->id]) }}"
                enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="card">
                    <div class="card-header p-3 bg-white">
                        <h4 class="card-title my-auto">Ubah Surat Masuk</h4>
                    </div>
                    <div class="card-body p-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="name"
                                placeholder="Judul Surat" value="{{ $incoming_mail->name }}" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="classification" name="classification">
                                ...
                                @foreach ($classifications as $classification)
                                    <option value="{{ $classification->id }}"
                                        @if ($incoming_mail->classification_id == $classification->id) selected @endif>
                                        {{ $classification->name }}
                                    </option>
                                @endforeach
                            </select>
